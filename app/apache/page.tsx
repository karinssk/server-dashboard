...
import { Globe, Activity, FileText, AlertTriangle, CheckCircle, XCircle, Edit, Trash2, Plus, Play, Terminal, X, Save, RotateCw } from 'lucide-react';
import { useState } from 'react';
import Swal from 'sweetalert2';
import ApacheLogViewer from '../components/ApacheLogViewer';
...
export default function ApachePage() {
    const { data: status, error: statusError } = useSWR<ApacheStatus>('/api/apache/status', fetcher);
    const { data: config, error: configError } = useSWR<ApacheConfig>('/api/apache/config', fetcher);
    const { data: sitesData, error: sitesError } = useSWR<SiteListResponse>('/api/apache/sites', fetcher);
    ...
    const handleEditSite = async (name: string) => {
        setLoadingSite(true);
        try {
            const res = await fetch(`/api/apache/site?name=${name}`);
    ...
    const handleCreateSite = () => {
        setEditingSite({ name: '', content: '<VirtualHost *:80>\n    ServerAdmin webmaster@localhost\n    DocumentRoot /var/www/html\n    ServerName example.com\n\n    ErrorLog ${APACHE_LOG_DIR}/error.log\n    CustomLog ${APACHE_LOG_DIR}/access.log combined\n</VirtualHost>' });
        setIsNewSite(true);
    };
    ...
    const handleSaveSite = async () => {
        ...
        setSavingSite(true);
        try {
            const res = await fetch('/api/apache/site', {
                method: 'POST',
    ...
    const handleDeleteSite = async (name: string) => {
        ...
        if (result.isConfirmed) {
            try {
                const res = await fetch(`/api/apache/site?name=${name}`, { method: 'DELETE' });
    ...
    const handleRestartApache = async () => {
        setRestarting(true);
        try {
            const res = await fetch('/api/apache/action', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            Toast.fire({ icon: 'success', title: 'Apache restarted successfully' });
            mutate('/api/apache/status');
        } catch (error: any) {
            Toast.fire({ icon: 'error', title: 'Failed to restart Apache', text: error.message });
        } finally {
            setRestarting(false);
        }
    };
    ...
    const handleSaveConfig = async () => {
        ...
        try {
            const res = await fetch('/api/apache/config', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            mutate('/api/apache/config');

            // Ask to restart
            const result = await Swal.fire({
                title: 'Configuration Saved',
                text: 'Do you want to restart Apache to apply changes?',
                icon: 'success',
                showCancelButton: true,
    ...
    if (statusError) return <div className="p-8 text-red-500">Failed to load Apache status.</div>;
    if (!status) return <div className="p-8 text-gray-400 animate-pulse">Loading Apache status...</div>;
            ...
            <div className="p-8 max-w-7xl mx-auto">
                <header className="mb-8">
                    <h1 className="text-3xl font-bold bg-gradient-to-r from-orange-400 to-red-500 bg-clip-text text-transparent flex items-center gap-3">
                        <Globe className="text-orange-400" /> Apache Manager
                    </h1>
                </header>
                ...
                <div className="bg-gray-900 border border-gray-800 rounded-xl p-12 text-center">
                    ...
                    <h2 className="text-2xl font-bold text-white mb-2">Apache Not Detected</h2>
                    <p className="text-gray-400 max-w-md mx-auto">
                        We couldn't find an Apache installation on this system. Please ensure Apache is installed and available in your system PATH or standard locations.
                    </p>
                    <div className="mt-8 p-4 bg-gray-950 rounded-lg border border-gray-800 inline-block text-left">
                        ...
                        <code className="text-sm text-orange-400 font-mono">brew install httpd</code>
                    </div>
                </div>
            </div>
        );
            ...
            <header className="mb-8 flex justify-between items-center">
                ...
                <div className="flex gap-3">
                    ...
                    >
                        ...
                        {restarting ? 'Restarting...' : 'Restart Apache'}
                    </button>
                    <button
                    ...
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            ...
                            <p className="text-xl font-bold text-white truncate" title={status.version}>
                                {status.version?.split('Server version: ')[1]?.split(' ')[0] || status.version || 'Unknown'}
                            </p>
                        </div>
                    </div>
            ...
            <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden flex flex-col h-[300px]">
                <div className="p-4 border-b border-gray-800 flex items-center justify-between bg-gray-950">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-sm font-semibold text-white">Main Configuration (httpd.conf)</h2>
                        {config?.path && (
            ...
            {showLogs && (
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="w-full max-w-5xl h-[80vh]">
                        <ApacheLogViewer onClose={() => setShowLogs(false)} />
                    </div>
                </div>
            )}
        </div>
